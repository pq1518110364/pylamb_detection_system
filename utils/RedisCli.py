...
class RedisCli(Singleton):
    ...
    def __init__(self) -> None:
        '''初始化'''
        self._connect_url = settings.CACHES['redis_cli']['LOCATION']  # settings.CACHES['redis_cli']['LOCATION'] 用连接池方式 注意设置账号密码方式
        self.current_db = int(self._connect_url[-1])
        password = settings.CACHES['redis_cli']['OPTIONS']['PASSWORD']
        pool = redis.ConnectionPool().from_url(self._connect_url)
        self.coon = redis.Redis(connection_pool=pool
                                , password=password  # 用连接池方式，这种设置密码无效
                                , socket_timeout=60)  # 连接密码设置 数据连接池
...
class RedisHash(object):
    ...
    def __repr__(self) -> str:
        to_dict = {key: self.get(key) for key in
                   map(lambda x: x.decode(), self._conn.hkeys(self._data_key))} if not self.is_empty else {}
        return str(to_dict)
